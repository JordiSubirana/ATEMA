...
{
	//PRIVATE
	Flags File::process_options(Flags opts) const noexcept
	{
		Flags buffer = opts;
	...
	File::File(const char *filename, Flags opts)
	try :
		m_file(),
		m_valid(false),
		m_opts(0),
	...
	void File::open(const char *filename, Flags b_opts)
	{
		try
		{
			...
			{
				ATEMA_ERROR("File could not be open.")
			}
		}
		catch (const std::exception& e)
		{
	...
	void File::write(const char *format, std::size_t max_size, ...)
	{
		va_list args;
	...
	bool File::end_of_file() const noexcept
	{
		return (m_eof);
	}
	
	std::string File::get_line(int count)
	{
		std::string tmp;
	...
	int File::get_current_line_index()
	{
